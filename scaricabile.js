...
PARTE 2: 
Nella pagina HTML, inserisci 2 input di tipo testo (uno per la location e uno per il titolo lavorativo, ricordati di diversificarli con 
  un id) e un bottone con valore “cerca”
...
function ricerca() {
  ...
  //LI METTO NELLA FUNZIONE CREATA PER ELABOLARLI
  ...
  if (finale && finale.posizione.length > 0) {
    ...
    finale.posizione.forEach((banane) => {
      ...
      divResult.appendChild(listaOfferte);
    });

    let conteggio = document.getElementById("conteggio");
    conteggio.innerHTML =
      "Elementi corrispondenti alla ricerca:" + finale.counter;
  } else {
    const messaggio = document.createElement("p");
    messaggio.innerHTML = "";
    messaggio.textContent = "NON HO TROVATO OFFERTE";
    messaggio.id = "nessunRisultato";
    divResult.appendChild(messaggio);
  }
